...
public class Game {
    ...
    Game(soundManager s) {
        ...
        ImageIcon bgImage = new ImageIcon(Objects.requireNonNull(getClass().getResource("background.jpg")));
    ...
    public void startGame(Game_mode m) throws GameException {
        try {
            ...
            if (m instanceof Challenge_mode){
                    ...
                    colorButton.addActionListener(e -> {
                        if (c.trim().equalsIgnoreCase(mc.getAnswer().trim())) {
                            ...
                            if (gfm.getWrongAttempts() >= GameFlowManager.MAX_WRONG_ATTEMPTS) {
                                return; // Stop further action if game is over
                            }
                            feedback.setText("Wrong! Score decreased by 1");
                            feedback.setVisible(true);


                            Timer feedbackTimer = new Timer(1500, evt -> feedback.setVisible(false));
                ...
                colorLabel.setText(mc.getText());
                Color classicColor = mc.getColor();
                if (classicColor.equals(Color.BLACK)) {
                    colorLabel.setForeground(Color.LIGHT_GRAY);
                } else {
                    colorLabel.setForeground(classicColor);
                }
                for (int i = 0; i < colorNamesList.size(); i++) {
                    String c = colorNamesList.get(i);
                    ...
                    colorButton.setBorderPainted(true); // adds border on the buttons
                    ...
                    colorButton.addActionListener(e -> {
                        if (c.trim().equalsIgnoreCase(mc.getAnswer().trim())) {
                            scoreBoard.increaseScore();
                            gfm.resetWrongAttempts();
                            feedback.setVisible(false); // hide feedback if it was showing
                        } else {
                            scoreBoard.decreaseScore();
                            gfm.recordWrongAttempt();
                            if (gfm.getWrongAttempts() >= GameFlowManager.MAX_WRONG_ATTEMPTS) {
            ...
            feedback.setFont(new Font("Arial", Font.BOLD, 24));
            feedback.setForeground(Color.RED);
            feedback.setVisible(false);  // hidden by default
