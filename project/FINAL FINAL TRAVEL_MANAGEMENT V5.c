...
{
    int departureChoice, destinationChoice, budgetChoice, airlinechoice, adults, children;
    char date[20];
    FILE *fp;
    ...
    if(fp!=NULL)
    {
	    ...
	    {
            printf("Invalid city choice.Exiting...\n");
            return;
        }
        else if(departureChoice==destinationChoice)
	    {
            printf("Departure and destination cities cannot be the same. Please try again.\n");
            return;
        }
        else
        {
...
void bookRoundTripFlight(char name[],const char *cities[],const char *Airlines[3][3],int AirlinePrices[3][3])
{
    int destinationChoice,departureChoice, budgetChoice, airlineChoice, adults, children;
    char departureDate[20], returnDate[20];
    FILE *fp;
    fp=fopen("Travel_management.txt","a");
if(fp!=NULL)
{
    ...
    if(departureChoice<1 || departureChoice>cityCount || destinationChoice<1 || destinationChoice>cityCount)
	{
        printf("Invalid city choice. Please try again.\n");
        return;
    }
...
{
    char re_pass[50];
    int account,M_attempts=0;
    printf("Do you have an account?(choose 1 or 2)\n 1. No\n 2. Yes\n");
    scanf("%d",&account);
    getchar();
    ...
    if(filepointer!=NULL)
    {
        switch(account)
		{
            case 1:
				...
				fgets(M1.email,sizeof(M1.email),stdin);
                M1.email[strcspn(M1.email,"\n")]=0;
                do{
                    if(!isValidEmail(M1.email) && M1.e_attempts<2)
					{
                ...
                }while((!isValidEmail(M1.email))&& M1.e_attempts<2);
                fflush(stdin);
              
                printf("Enter your Username:\n");
                fgets(M1.name,sizeof(M1.name),stdin);
                M1.name[strcspn(M1.name,"\n")]=0;
                if(!(IsValiduserName(M1.name)))
                {
                	return;
				}
                ...
                fgets(M1.re_pass,sizeof(M1.re_pass),stdin);
                M1.re_pass[strcspn(M1.re_pass,"\n")]=0;
                
                while(strcmp(M1.re_pass,M1.pass)!=0 && M_attempts<2)
                {
                    printf("Passwords do not match.Please re-enter the password.You are currently using the attempt %d out of 2\n",M_attempts+1);
                    scanf("%s",M1.re_pass);
                    M_attempts++;
                    if(strcmp(M1.re_pass,M1.pass)!=0 && M_attempts==2)
                    {
                	    printf("\nMaximum Attempts reached.Exiting...");
                	    return;
                    }
                ...
                fprintf(filepointer,"\t\tMANAGER:\nEmail:%s\nUsername:%s\n",M1.email,M1.name); 
                printf("ACCOUNT CREATED SUCCESSFULLY! \nWELCOME Manager %s ON BOARD!\n",M1.name);
                fprintf(filepointer,"Hi %s!\nWELCOME ON BOARD",M1.name);
                fclose(filepointer);
                dashboard(M1);
                break;
            ...
            case 2:
                ...
                printf("Hi Manager %s!, you're logged in.\n", M1.name);
                fprintf(filepointer,"Hi Manager %s!, you're logged in.\n",M1.name);
                fclose(filepointer);
                dashboard(M1);
                break;
